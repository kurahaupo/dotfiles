...
aws_show_cloudfronts ()
{
  aws cloudfront list-distributions |
   jq -r '  .DistributionList |
            .Items [] |
            "\(.Id) \(.DomainName) \(.Origins.Items[0].DomainName) \(.Status)"'
}
aws_show_load_balanacers()
{
  aws elb describe-load-balancers |
   jq ' .LoadBalancerDescriptions [] |
        "\( .DNSName ) \( .Instances [] | join(",") )"'
}
...
aws_update_ssh_hosts()
{
  aws ec2 describe-instances |
    jq -r ' .Reservations[] .Instances [] |
            "\(.PublicDnsName) \(.KeyName) \((((.Tags//[])[]|select(.Key == "Name"))//{}).Value) \(.InstanceId)"
          ' |
  while read hostname keyfile name instanceid; do
    if [[ $name = null ]]; then
...
aws_update_ssh_from_home()
{
  ...
  local oldip=$( aws ec2 describe-security-groups  --group-names $sec_group --filters Name=ip-permission.from-port,Values=22 --query 'SecurityGroups[].IpPermissions[].IpRanges[].CidrIp' |
                 jq -r '.[]' |
                 grep -oE '([0-9]+\.?){4}' |
                 xargs )
...
aws_show_pending_spot_instances()
{
  aws ec2 describe-spot-instance-requests |
   jq '.SpotInstanceRequests[]|select(.Status.Code != "fulfilled")|{Status: .Status.Message, SpotInstanceRequestId }'
}
aws_show_spot_instance_requests()
{
  aws ec2 describe-spot-instance-requests |
   jq '.SpotInstanceRequests[]|{Status: .Status.Message, SpotInstanceRequestId }'
}
aws_copy_security_groups_to_instance()
{
  ...
  local groups=$( aws ec2 describe-instances --instance-ids $instance_id |
                  jq '{ Groups: [.Reservations[].Instances[].SecurityGroups[].GroupId] }'
                )
...
aws_show_security_group()
{
  ...
  if [[ -z $security_group_name ]]; then
    aws ec2 describe-security-groups |
     jq '.SecurityGroups[]|{GroupName,IpPermissions,IpPermissionsEgress}'
    return
  fi
  aws ec2 describe-security-groups |
   jq " .SecurityGroups [] |
        ...
        select( .GroupName == \"$security_group_name\") "
}
...
aws_show_policies()
{
  aws iam list-policies |
   jq ' .Policies [] |
        ( .Arn | select(startswith("arn:aws:iam::")))'
}
...
aws_set_profile()
{
  ...
  if [[ -z $profile_id ]]; then
    ...
    awk '/^\[/ { gsub(/\[|\]/, "", $1); print $1 }' < ~/.aws/credentials
    return
  fi
  local profile=$( grep -E "\[.*$profile_id.*\]" ~/.aws/credentials |
                   sed -e 's/\[//' -e 's/\]//' -e '/^$/d' )
  if [[ $profile = *$'\n'* ]]; then
  ...
  keys=$( grep -A2 $profile ~/.aws/credentials |
          tail -n2 |
